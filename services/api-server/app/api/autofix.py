"""
API endpoints for autofix operations - Story 3.1 & 3.2:
Secret Generation and Database Setup (Auto-Fix)
...
class AutofixSecretsResponse(BaseModel):
    ...
    encryption_key_added: bool = Field(description="Whether encryption key was generated and added")
...
class AutofixDatabaseResponse(BaseModel):
    ...
    configs_created: list[str] = Field(description="List of configuration keys that were created")
    message: str = Field(description="Human-readable result message")
    timestamp: str = Field(description="ISO timestamp of the operation")
...
def autofix_secrets() -> AutofixSecretsResponse:
    ...
    if not env_path.exists():
        raise HTTPException(
            status_code=404, detail=f".env file not found at {env_path}. Please create it first."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=403, detail=f"Permission denied when writing to .env file: {e}"
        ) from e
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to autofix secrets: {e}") from e
...
def autofix_database(db: DbSession) -> AutofixDatabaseResponse:
    """
    ...
    try:
        ...
        if result["success"]:
            if result["admin_created"]:
                message = (
                    "Database setup completed. Admin user created with email: "
        ...
        raise HTTPException(status_code=500, detail=f"Failed to autofix database setup: {e}") from e
