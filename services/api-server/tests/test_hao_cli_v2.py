...
def test_hao_v4_slash_menu_filters_and_hints_look_claude_like(
    tmp_path: Path,
    monkeypatch,
) -> None:
    app = _make_hao_app(tmp_path, monkeypatch)
...
def test_hao_v42_welcome_card_and_workbench_start_claude_like(
    tmp_path: Path,
    monkeypatch,
) -> None:
    app = _make_hao_app(tmp_path, monkeypatch)
...
def test_hao_v4_alias_commands_route_to_model_permissions_usage_and_context(
    ...
    app._handle_command("/model anthropic/claude-sonnet-4-5")
