...
export function ChatWelcomeState({
  ...
  const { text } = useI18n();
  const intro = text(
    "这是 Harness 的 Agent Workspace。像 local Agent CLI / Harness Agent 一样和 Agent 对话，工具调用与运行细节会同步到右侧 Inspector 或 Run 详情页。",
    "This is Harness's Agent Workspace. Chat with the agent like local Agent CLI or Harness Agent — tool calls and run details stream to the Inspector drawer or Run Detail page.",
  );
