...
- Local Agent Tool Safety V3 is planned and double-reviewed on 2026-06-04 on branch `feature/local-agent-tool-safety-v3`: `.omx/plans/prd-local-agent-tool-safety-v3.md` and `.omx/plans/test-spec-local-agent-tool-safety-v3.md` are入库; Aristotle architecture review returned `PASS`; Parfit security/test review returned `PASS`. V3 implementation must make bridge-reported side-effect results authorization-bound, keep local host approvals separate from server ToolRunner execution, add deny-by-default server classification, command lifecycle, pending change, cancel/retry, audit fail-closed, and deterministic fake/hao smoke coverage before code review and push.
