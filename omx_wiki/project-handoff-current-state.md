...
- Local Agent Claude Code Permission Bridge V6 is implemented and verified locally on branch `feature/local-agent-claude-code-permission-bridge-v6`: `.omx/plans/prd-local-agent-claude-code-permission-bridge-v6.md` and `.omx/plans/test-spec-local-agent-claude-code-permission-bridge-v6.md` remain the authoritative plan docs; Claude Code V6 is now an opt-in Agent SDK permission bridge path that captures host-tool intent and routes it through existing V3 `LocalAgentToolRequest`, `ToolApproval`, `ToolCall`, `AgentEvent`, pending-change, and command lifecycle authority before host side effects. V6 is normalized to `permission_bridge=harness_local_tool_request_v1`, `execution_mode=agent_sdk_intent_capture_harness_executor`, `permission_bridge_execution=harness_owned_executor`, and `sdk_native_tool_execution_enabled=false`, so the Claude SDK callback returns deny after Harness-owned execution and cannot perform duplicate native side effects. Scoped pairing entitlement plus normalized capabilities are both required to enable V6; V5 Claude connections cannot self-upgrade to V6 through heartbeat and remain the conservative no-tools fallback. Modified approval now updates the executable input hash, requires `write_file` and `apply_patch` pending-change preview refresh against the approved modified input, fail-closes stale original preview results, and records the refreshed preview on both the local request and approval audit path before commit. Task cancel now terminalizes pending V6 local-tool state, including approvals, tool calls, pending changes, commands, and bridge tasks. V1-V6 cross-review is closed: architecture/protocol remained `CLEAR`; test WATCH was resolved by exact deterministic smoke scenarios for approve write, reject bash, revoke pending, approval timeout, bypass attempt, and API failure fail-closed; design WATCH was resolved by Workspace Claude V5/V6 labels, Claude V6 approval pending state, and template-scoped `使用此模板` / `Use template` copy. Validation passed V6 smoke coverage for SDK unavailable, approve write, modified approval, reject bash, revoke pending, approval timeout, bypass attempt, API failure fail-closed, approved shell execution, cancelled execution, and V5 heartbeat upgrade denial; targeted backend regression (`12 passed, 34 deselected`), targeted Claude CLI regression (`11 passed, 39 deselected`), targeted Ruff, frontend local-agent tests (`23 passed`), frontend typecheck, docs validation, and diff check. Final implementation review returned architecture `CLEAR` and code/security `APPROVE`. Native Claude resume, deferred tool resume after process exit, live credential UX, remote-control/web/cloud sessions, MCP/plugins/hooks/subagents/browser/computer-use, and server-side Claude credential handling remain out of scope.
...
- Local Agent Codex Adapter V4 is verified locally on branch `feature/local-agent-codex-adapter-v4`: `.omx/plans/prd-local-agent-codex-adapter-v4.md` and `.omx/plans/test-spec-local-agent-codex-adapter-v4.md` are入库; `adapter_kind=codex` is supported while `claude_code` remains future scope; Agent Studio generates adapter-scoped Codex pairing commands; Codex capabilities are server-normalized to no native resume, no cancel, no host tools, and forced `context_replay_new_session`; `hao bridge pair/run --adapter codex` uses stdin prompt input, isolated subprocess env, read-only Codex sandbox args, sidecar-only raw workspace root storage, and split device-token storage; Codex legacy `tool_result` is rejected before receipt or `ToolCall`; Codex workspace execution compares local sidecar hash with the server task `workspace_identity_hash` before spawning. Deterministic smoke covers unavailable, read-only reply, forced context replay, and side-effect rejection. Final implementation review fixed one architecture BLOCK for mutable local workspace identity; final architecture review returned WATCH with no blockers after native-resume test-spec wording was aligned to V4 context replay, and final code/security review returned APPROVE after the stale EOF whitespace blocker was fixed.
