...
- Local Agent Claude Code Adapter V5 is planned and review-approved on branch `feature/local-agent-claude-code-adapter-v5`: `.omx/plans/prd-local-agent-claude-code-adapter-v5.md` and `.omx/plans/test-spec-local-agent-claude-code-adapter-v5.md` are入库; `adapter_kind=claude_code` is planned as a conservative assistant-response local-agent adapter using existing pairing, bridge task, Run/Event/Message, Workspace ChatSurface, pending/offline, and API/DB authority paths. Required command shape is `claude --bare -p --output-format stream-json --verbose --include-partial-messages --no-session-persistence --permission-mode default --tools ""`; V5 forbids host tools, native resume, MCP/plugin/hook/subagent/browser/remote-control surfaces, `--settings`, `--setting-sources`, `apiKeyHelper`, and permission bypass modes in the required path. Claude Code subprocesses must use stdin/private `0600` input, temp `HOME`, temp `CLAUDE_CONFIG_DIR`, private cwd, no paired workspace file exposure, redacted local-only credential handling, and `system/init` safety proof before assistant success projection. Architecture/protocol and security/test reviewers both returned `PASS`. V5 is not implemented yet.
