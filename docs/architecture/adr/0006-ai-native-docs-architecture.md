...
The harness documentation is a brownfield system: `docs/development/ai/` already holds a
...
`docs/architecture/adr/` (records 0001-0005). CI already enforces documentation invariants
...
1. **Reuse `docs/architecture/adr/`, do not create `docs/decisions/`.** The repo already has
...
2. **Use JSON for the module map (`docs/architecture/module-map.json`), not YAML or TOML.**
   The repo already parses `docs/development/ai/context-index.json` with the stdlib `json`
...
3. **Generate `docs/architecture/MODULE-INDEX.md` from `docs/architecture/module-map.json`; do not
   hand-author it.** A new `--gen-module-index` flag on
   `scripts/agent-context-brief.py` renders the index deterministically from
   the map. CI guards sync with `git diff --quiet docs/architecture/MODULE-INDEX.md`.
   `docs/contracts/SPEC-INDEX.md` remains human-authored (curated, feature-keyed) because
...
4. **The session-end (Stop) hook is append-only and never calls an LLM.** Each
   ...
   `docs/development/ai/session-log-pointers.md`. No model call, no network. The brief
...
- `docs/architecture/module-map.json` must be kept truthful: any real refactor that moves a
  module path requires updating the map (enforced loudly by CI).
- `docs/architecture/MODULE-INDEX.md` must only ever be regenerated, never hand-edited.
