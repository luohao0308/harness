...
### Desktop Electron launch failure
...
```text
clear ELECTRON_RUN_AS_NODE before real Electron launches
set NO_PROXY=localhost,127.0.0.1 when testing local API/Vite paths
if failures appear after electron-builder --dir, rebuild better-sqlite3 for Node ABI before rerunning Node/Vitest tests
use docs/development/desktop/README.md for the full desktop support playbook
```

### Platform provider key vs mock fallback

Check:

```bash
grep -n "AI_PROVIDER_\\|MODEL_GATEWAY_BASE_URL" deploy/docker-compose/.env.example .env.example services/api-server/.env.example
```

Action:

```text
AI_PROVIDER_API_KEY must be present only in the backend deployment environment
or Helm Secret when exercising the platform provider; never commit or expose it
to browser environment files
production startup fails when AI_PROVIDER_API_KEY is empty or uses the example
placeholder; this prevents silent mock output in production
AI_PROVIDER_PROTOCOL is fixed to chat_completions and
AI_PROVIDER_BASE_URL=https://chybenzun.top/v1 is the product default; the
responses protocol is not implemented in Harness
MODEL_GATEWAY_* and DEEPSEEK_API_KEY are legacy local mock compatibility
settings; successful development output without AI_PROVIDER_API_KEY is not a
real upstream-provider verification
```
