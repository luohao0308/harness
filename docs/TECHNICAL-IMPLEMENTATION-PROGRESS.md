...
| 技术项 | 目标 | 当前状态 | 证据 |
|---|---|---|---|
| 官网 | Next.js | 已落地官网工程、产品页面和公开 OpenAPI 入口 | `apps/web-site` |
| 控制台 | React + Vite | 已落地主要运行页面和中文默认体验 | `apps/agent-console` |
...
| 异步任务 | Dramatiq | 已落地 | `services/api-server/app/workers/subagent_worker.py` |
...
| Replay | snapshot + event replay | 已落地 | `POST /api/tasks/{task_id}/replay` |
| 模型网关 | OpenAI-compatible | 已落地 | Model Gateway、model_calls、fallback 观测 |
| 工具治理 | Tool Registry + Policy Engine | 已落地 | Tool Runner、tool_calls、策略拒绝和沙箱路径 |
...
| 同步执行 | Executor 直接执行 step | `GET /api/tasks/{task_id}/steps`、`GET /api/tasks/{task_id}/events` | 已落地 |
| 异步执行 | async step 派生 Subagent | `GET /api/tasks/{task_id}/plan`、`GET /api/tasks/{task_id}/subagents`、`POST /api/tasks/{task_id}/subagents` | 已落地，已请求 Dramatiq 入队 |
...
| Subagent 工具链 | worker 执行 assignment 内工具并审计 | `GET /api/tasks/{task_id}/tool-calls`、`GET /api/tasks/{task_id}/result` | 已落地 |
| 工具执行 | 按策略执行工具 | `POST /api/tasks/{task_id}/tools/execute` | 已落地 |
| 模型调用审计 | 查询供应商、模型、token、延迟、失败 | `GET /api/tasks/{task_id}/model-calls` | 已落地 |
...
|---|---|---|---|---|
| 同步执行 | Executor 执行 `execution_mode=sync` 的 step | 任务详情执行计划展示“同步执行”和步骤状态 | `task_steps.execution_mode=sync`、`STEP_STARTED`、`STEP_COMPLETED` | 已落地 |
| 异步执行 | Executor 对 `execution_mode=async` 且 `can_spawn_subagent=true` 的 step 派生 Subagent | 执行计划展示“异步执行”、子 Agent ID 和状态，Subagent 面板展示来源 step | `task_steps.assigned_agent_id`、`agent_runs.status`、`SUBAGENT_SPAWNED` | 已落地 |
| 并发控制 | Subagent 上限固定 5 | 页面展示任务最大子 Agent 数 | `tasks.max_subagents`、`agent_runs` | 已落地 |
| 超时保护 | worker 使用 timeout | 子 Agent 列表展示 timeout 时间 | `agent_runs.timeout_at`、`SUBAGENT_TIMEOUT` | 已落地 |
...
| `/observability` | 运行摘要 | Observability Summary、Metrics | 已接入 |
...
| Loki | 已落地 | `GET /api/observability/logs`、Promtail 采集、标签检索 | 保持日志检索深链回归 |
