...
本文件只保留事件、Event Store 和 API 契约指引。数据表字段以 [database-schema.yaml](./database-schema.yaml) 为唯一机器契约，API 以 [openapi.yaml](../../../contracts/api/openapi.yaml) 为唯一机器契约。

## 数据契约

```text
机器契约：docs/development/ai/reference/database-schema.yaml
说明文档：docs/development/ai/reference/database-erd-migrations.md
```

## API 契约

```text
机器契约：docs/contracts/api/openapi.yaml
说明文档：docs/contracts/api/openapi-contract.md
```
