...
- 计划完成后的 macOS 本地预检：在 macOS 14.4.1 x64 上重新构建 unsigned x64 目录包后，两轮独立五样本检查均按预期被门禁阻断。总启动 P95 分别为 `7314ms`、`6480ms`（预算 `6000ms`），服务就绪到渲染完成 P95 分别为 `4999ms`、`4828ms`（预算 `3500ms`）；另一次单样本为 `4095ms` 并通过，表明当前缺口是连续启动波动而非报告链路不可用。本地结果不替代正式 Release runner 证据。
