...
| Approve, Reject, Modify tool approvals | `docs/03-api-spec.md` exposes approve/reject/modify approval endpoints; `docs/08-console-ui-spec.md` requires tool approvals with approve, reject, and modify actions for admins | Backend endpoint and frontend API helper exist, but the console UI still lacks the Modify action in pending approval cards | 待实现 | `cd apps/agent-console && npm run build`; `cd services/api-server && uv run pytest tests/ -q` |
