...
2. 读取 docs/ai/00-execution-protocol.md。
3. 读取 docs/ai/01-task-progress.md。
4. 读取 docs/ai/task-progress.yaml。
...
12. 创建 Pull Request。
13. 更新 docs/ai/task-progress.yaml。
14. 更新 docs/human/10-task-progress.md。
15. 输出阶段完成摘要。
...
- PR 未合并前不得进入下一阶段。
- 每阶段完成后必须更新 docs/ai/task-progress.yaml。
- 每阶段完成后必须更新 docs/human/10-task-progress.md。
- 进度未更新时禁止进入下一阶段。
...
```text
按 docs/ai/00-master-prompt.md 的流程连续执行。从 task-progress.yaml 的 current_stage 开始，每完成一个阶段就验证、commit、push、创建 PR，并更新 docs/ai/task-progress.yaml 和 docs/human/10-task-progress.md。PR 未合并时停止。遇到 blocked、failed、缺少凭证、缺少外部账号、缺少 Docker 权限、依赖安装失败、测试失败时停止并输出阻塞原因。
```
