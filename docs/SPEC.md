...
- `codex_plan` returns markdown planning text for the user to review. It does not create an `ExecutionPlan`, tool call, approval, or artifact event by itself.
...
- `codex_plan` planning text renders as normal assistant content; explicit `plan` traces and thought-like text render as collapsible trace blocks when returned.
