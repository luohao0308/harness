...
- [x] 2. Wave 1 — Pure library functions
  ...
  - [x] 2.6 实现 `planApprovalGate.ts` 可见性纯谓词
    ...
    - **Acceptance**: `cd apps/agent-console && npm run lint && npm run build`
    - 导出 `planApprovalGate({ activePath, activeStreamNodeId, dismissedPlanNodeIds })`。7 项预条件逐项判定：length > 0 / tail role / tail state / workspace_mode ∈ {plan, codex_plan} / activeStream === null / tail 未被 dismiss。满足全部 → `{ visible: true, planNode: tail }`；任一不满足 → `{ visible: false, planNode: null }`。TOTAL：对任何 `activePath` 形状（含空数组、undefined metadata）都不得 throw。
...
| P8       | Metadata parity (Strip ≡ Drawer)                | 7.1–7.4, 16.8                   | 4.3, 6.1                     | 8.1      |
...
| c | Plan 审批浮层「批准并执行」           | 选 `codex_plan` 或 `plan` 模式，提交 "帮我规划一次 1 周 CI 加固"。待 plan 输出 `state=done`。           | Composer 正上方出现 Plan 审批浮层，四个按钮可见；点击「批准并执行」后浮层按钮 disable，开始新的 plan-act 流。     |
