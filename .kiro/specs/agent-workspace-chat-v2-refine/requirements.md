...
3. **Plan (markdown) 模式缺少 Codex / Claude Code 风格的规划审批浮层**：收到 plan 后应在 Composer 上方弹出"批准并执行 / 修改规划 / 丢弃"。
...
- **Plan_Approval_Panel**: 当最近一条 assistant `ConversationNode` 的 `metadata.workspace_mode ∈ {plan, codex_plan}` 且 `state = done` 时，显示在 `Workspace_Composer` 正上方的浮层卡片，含「批准并执行」、「修改规划」、「丢弃」、「关闭（X）」四个动作。
...
- **Workspace_Mode**: Workspace 支持的消息提交模式 `chat` / `codex_plan` / `plan`（语义沿用 v1）。
...
**User Story:** As 使用 Plan (markdown) 或 Plan-Act 模式的使用者, I want 在模型返回规划后，Composer 上方出现一个 Codex / Claude Code 风格的小浮层让我做"批准 / 补充 / 丢弃", so that 我可以快速决定这次规划是执行、改、还是丢弃，而不是回到普通聊天再手动发指令。

#### Acceptance Criteria

1. WHEN `Active_Path` 中最后一个 `ConversationNode` 满足 `role = assistant` 且 `state = done` 且 `metadata.workspace_mode ∈ {plan, codex_plan}`, THE Workspace SHALL 在 `Workspace_Composer` 正上方渲染 `Plan_Approval_Panel`。
...
4. WHEN 用户点击「修改规划」, THE Workspace SHALL 把该 plan 节点的 `content` 文本填入 `useWorkspaceStore.draft`，聚焦 `Workspace_Composer`，并隐藏 `Plan_Approval_Panel`；用户再次提交时按当前选择的 `Workspace_Mode`（通常仍为 `codex_plan` 或 `plan`）重新发起请求。
...
**User Story:** As Workspace 使用者, I want 用户消息气泡是白底黑字, so that 它在整体灰白主题下更清爽、更像主流 Chat 产品（ChatGPT、Claude、Gemini）的用户气泡。
...
4. **P4 Plan-panel precondition invariant**: THE Plan_Approval_Panel SHALL 仅在 `Active_Path` 末端 `ConversationNode` 满足 `role = assistant ∧ state = done ∧ metadata.workspace_mode ∈ {plan, codex_plan}` 且 `activeStream = null` 时可见；其他所有情况下 SHALL 不可见。
...
8. **P8 Metadata parity invariant**: THE Metadata_Strip 展示的 `input_tokens`、`output_tokens`、`cost_usd`、`ttfb_ms`、`duration_ms`、`run_id` 值 SHALL 在任意时刻等于 `Active_Path` 末端 `ConversationNode.metadata` 中对应字段的值（缺失字段以占位符显示），不存在 `Inspector_Drawer` 与 `Metadata_Strip` 展示数值不一致的时刻。
