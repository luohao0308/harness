...
- [x] 1. 建立 `features/agents` 子目录骨架与类型入口
  - **产出**：
    ...
    - 新建 `apps/agent-console/src/features/agents/lib/types.ts`：导出 `WorkspaceMode = "chat" | "codex_plan" | "plan"` 与 `InspectorSection = "metadata" | "artifacts" | "runtime"`
  - **依赖**：无
...
- [x] 15. 组装与清理：重写 `AgentWorkspacePage.tsx` ≤ 120 行
  ...
  - **Acceptance**：
    ...
    - 手动跑 `npm run dev` 并在 `/agents/default/workspace` 验证：
      ...
      3. 在 `chat ↔ codex_plan ↔ plan` 间切换，`Active_Path` 与 `draft` 均保留
...
3. **模式切换不清 draft**：在输入框输入文本但不发送，在 `chat ↔ codex_plan ↔ plan` 之间来回切换 → 输入框内容与已有对话历史均保持不变。
