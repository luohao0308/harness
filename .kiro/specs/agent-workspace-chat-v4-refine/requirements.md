...
7. **锦上添花优化** — 从一组候选里选 2–3 条最能提升"像 Claude Code / Codex"的体验进入 requirements：**代码块右上角 Copy code 按钮** / **流式期间 caret 动画光标** / **连续同角色消息 group-by-role**。其余（侧栏搜索、按日期分组、`Cmd+Enter` 独立发送键绑定、消息 timestamp hover）归入 Out of Scope 供未来迭代。
...
**User Story:** As Workspace 使用者, I want Workspace_Composer 单行起步高度在 24px 或以下, so that 首屏 Composer 不再有空白浪费、与 ChatGPT / Claude Code 的视觉比例更接近。
...
2. THE Top_Meta_Bar SHALL 不再渲染 `Workspace_Mode` badge（即无论 `workspaceMode` 取 `"chat" / "codex_plan" / "plan"` 任何值都隐藏；模式切换靠 v3 的 Slash 命令 `/chat` `/plan` `/codex` 表达）。
...
### Requirement 7: Claude Code / Codex 风格微调 / Claude-Code-like polish

> **用户反馈 (原话)**: 根据你的想法看看哪里还可以优化 — 列 3-5 条轻量改进 ... 从中选 2-3 条最能提升"像 Claude Code / Codex"的放进 requirements.

**User Story:** As Workspace 使用者, I want 对话界面有若干"Claude Code / Codex 式"的细节, so that 使用体验更接近行业标杆 CLI / Web 产品。
...
2. THE Workspace SHALL 继续支持 v3 的 `Slash_Menu` 全部 9 条命令（`/plan` `/codex` `/chat` `/pin` `/clear` `/model` `/tool` `/search` `/help`），包括方向键 / Enter / Esc / Tab 键绑定。
