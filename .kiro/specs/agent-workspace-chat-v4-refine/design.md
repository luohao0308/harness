...
7. **Claude Code / Codex 式微调** — 代码块 Copy 按钮、流式 caret 光标、同角色消息 group-by-role（3 条）。
...
v3 常量从 `[40, 200]` 收紧到 `[COMPOSER_MIN_HEIGHT_V4, COMPOSER_MAX_HEIGHT] = [24, 200]`；`lineHeight` 从 `leading-6`（24px）降到 `leading-5`（20px）；`padding-y` 从 `py-2`（8px/8px）降到 `py-0.5`（2px/2px）。24 = 20 + 2 + 2 刚好容纳单行，与 ChatGPT / Claude Code 的起步视觉对齐。
