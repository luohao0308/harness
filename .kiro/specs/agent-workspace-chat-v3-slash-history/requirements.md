...
4. **没有历史对话 / 像完整的 ChatGPT 对话窗口**：需要左侧可折叠的"历史对话"抽屉，列出本 Agent 下的 conversations，支持新建 / 切换 / 删除 / 当前高亮。
5. **用 `/` 命令触发模式和动作**：在 Composer 输入 `/` 弹出命令菜单（`/plan`、`/codex`、`/chat`、`/pin`、`/clear`、`/model`、`/tool`、`/search`、`/help`），方向键 + Enter 确认，Esc 关闭；命令执行后 **不把 `/command` 本身发给后端**（除非命令需要 content）。
...
- **Slash_Command**: Slash 菜单的一项，包含 `name`（`"plan" | "codex" | "chat" | "pin" | "clear" | "model" | "tool" | "search" | "help"` 等）、`aliases`（例如 `plan-md → codex`）、`args`（是否需要参数）、`description.zh / description.en`。
...
7. THE History_Panel SHALL 提供收起按钮（可选：通过顶部 X 或 ChatGPT 风格汉堡按钮），收起后宽度变为 0，`Chat_Surface` 主列填满剩余宽度；收起状态 SHALL 持久化到 `localStorage`（键 `harness.workspace.v3.<agentId>.historyPanelCollapsed`）。
...
2. THE Slash_Menu SHALL 包含以下命令（双语 description）：
   ...
   - `/codex` （别名 `/plan-md`）→ 切换 `Workspace_Mode = "codex_plan"`（Plan markdown）。
...
3. THE ChatSurface SHALL 不再渲染 `ChatModeBanner`（v3 靠 `/chat`、`/plan`、`/codex` slash 命令 + 顶栏 mode badge 表达当前模式；`ChatModeBanner` 组件文件保留但不再从 `ChatSurface` 挂载，以减少 diff 面积）。
4. THE TopMetaBar 的 `Workspace_Mode` badge 在 `Workspace_Mode = "chat"` 时 SHALL 隐藏；仅在 `plan` / `codex_plan` 时显示。
